...
typedef struct {
    ...
    int xres, yres;
    unsigned int *thread_image;
    unsigned int *max_count;
    pthread_mutex_t *max_count_mutex;
} thread_data_t;

void enqueue(work_queue *queue, int degree) {
...
void *compute_zeroes(void *thread_data) {
    ...
    while (1) {
        int d = dequeue(data->queue);
        if (d == -1) {
            break; // No more work items
        }

        double poly[d+1];
        for (int j = 0; j <= d; j++) { poly[j] = data->coeff[0]; }
        int counter[d+1];
                ...
                if (status == 0) {
                    for (int i = 0; i < d; i++) {
                        ...
                        if (0 <= x && x < data->xres && 0 <= y && y < data->yres) {
                            int c = ++data->thread_image[data->xres * y + x];
                            if (max_count_local < c) { max_count_local = c; }
                        }
                    }
                }
            }
            for (j = 0; j <= d && counter[j] == data->ci_max-1; j++) { counter[j] = 0; poly[j] = data->coeff[0]; }
            if (j <= d) { counter[j]++; poly[j] = data->coeff[counter[j]]; }
        } while (j <= d);
        gsl_poly_complex_workspace_free(w);
        fprintf(stderr, "Thread %d: Completed degree %d\n", data->thread_id, d);
    }
...
int main(int argc, char* argv[]) {
    ...
    unsigned int *thread_images[num_threads];
    for (int t = 0; t < num_threads; t++) {
        thread_images[t] = (unsigned int *)calloc(sizeof(unsigned int), yres * xres);
    }

    work_queue queue = { NULL, PTHREAD_MUTEX_INITIALIZER };
    for (int di = 0; di < di_max; di++) {
        enqueue(&queue, degree[di]);
    }
    ...
    for (int t = 0; t < num_threads; t++) {
        ...
        thread_data[t].thread_image = thread_images[t];
    ...
    unsigned int *final_image = (unsigned int *)calloc(sizeof(unsigned int), yres * xres);
    for (int t = 0; t < num_threads; t++) {
        for (int y = 0; y < yres; y++) {
            for (int x = 0; xres > x; x++) {
                final_image[y * xres + x] += thread_images[t][y * xres + x];
            }
        }
        free(thread_images[t]);
    }
    ...
    for (int y = 0; y < yres; y++) {
        for (int x = 0; x < xres; x++) {
            ...
            unsigned int k = (unsigned int)(r * log((float)final_image[xres * y + x]));
            color[0] = k >> 8;
            color[1] = k & 255;
            color[2] = k >> 8;
            color[3] = k & 255;
